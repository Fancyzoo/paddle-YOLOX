...
from paddle.fluid.layers import multiclass_nms
...
def decode_outputs(outputs, input_shape):
    ...
    outputs = paddle.concat(x=[x.flatten(start_dim=2) for x in outputs], axis=2).permute(0, 2, 1)
    ...
    for h, w in hw:
        ...
        grid = paddle.stack((grid_x, grid_y), 2).view(1, -1, 2)
        shape = grid.shape[:2]

        grids.append(grid)
        strides.append(paddle.full((shape[0], shape[1], 1), input_shape[0] / h))
    # ---------------------------#
    #   将网格点堆叠到一起
    # ---------------------------#
    grids = paddle.concat(x=grids, axis=1).type(outputs.type())
    strides = paddle.concat(x=strides, axis=1).type(outputs.type())
    # ------------------------#
    #   根据网格点进行解码
    # ------------------------#
