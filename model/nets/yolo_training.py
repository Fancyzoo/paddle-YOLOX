...
class IOUloss(nn.Layer):
    ...
    def forward(self, pred, target):
        ...
        en = (tl < br).type(tl.type()).prod(axis=1)
...
class YOLOLoss(nn.Layer):
    def __init__(self, num_classes, strides=[8, 16, 32]):
        ...
        self.grids = [paddle.zeros([1])] * len(strides)
    ...
    def forward(self, inputs, labels=None):
        ...
        for k, (stride, output) in enumerate(zip(self.strides, inputs)):
            ...
            expanded_strides.append(paddle.ones_like(grid[:, :, 0]) * stride)
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                num_fg += num_fg_img
                cls_target = F.one_hot(gt_matched_classes.to_tensor(dtype=paddle.int64),
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
        ...
        cls_preds_ = cls_preds_.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_() * obj_preds_.unsqueeze(0).repeat(
            num_gt, 1, 1).sigmoid_()
        gt_cls_per_image = F.one_hot(gt_classes.to(paddle.int64), self.num_classes).float().unsqueeze(1).repeat(1,
                                                                                                                num_in_boxes_anchor,
                                                                                                                1)
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        dynamic_ks = paddle.clip(topk_ious.sum(1).int(), min=1)
...
def weights_init(net, init_type='normal', init_gain=0.02):
    def init_func(m):
        ...
        if hasattr(m, 'weight') and classname.find('Conv') != -1:
            if init_type == 'normal':
                m.weight.data = paddle.nn.initializer.Normal( 0.0, init_gain)
            elif init_type == 'xavier':
                m.weight.data = paddle.nn.initializer.XavierNormal( gain=init_gain)
            elif init_type == 'kaiming':
                m.weight.data = paddle.nn.initializer.KaimingNormal( a=0, mode='fan_in')
            else:
                raise NotImplementedError('initialization method [%s] is not implemented' % init_type)
        elif classname.find('BatchNorm2d') != -1:
            m.weight.data = paddle.nn.initializer.Normal( 1.0, 0.02)
            m.bias.data = paddle.nn.initializer.Constant( 0.0)
